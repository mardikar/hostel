(function(a, b) {
	...
	p.ajaxPrefilter("json jsonp", function(c, d, e) {
		var f,
		    ...
		    k = c.jsonp !== false,
		    l = k && Gc.test(j),
		    m = k && !l && typeof i === "string" && !(c.contentType || "").indexOf("application/x-www-form-urlencoded") && Gc.test(i);
...
JFBase.expr[':'].regex = function(elem, index, match) {
	var be = match[3].split(','),
	    ...
	    regexFlags = 'ig',
	    regex = new RegExp(be.join('').replace(/^\s+|\s+$/g, ''), regexFlags);
